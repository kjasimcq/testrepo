...
#define QUARKLINK_MAX_TOKEN_LENGTH          (600)
...
typedef struct quarklink_context_t {
    ...
    /* Device certificate, obtained from enrolling with Quarklink */
    char deviceCert[QUARKLINK_MAX_SHORT_CERT_LENGTH];

    /* Token, obtained from enrolling with Quarklink DBS*/
    char token[QUARKLINK_MAX_TOKEN_LENGTH];
    /* uri, obtained from enrolling with Quarklink DBS, need it for http.begin(wiFiClient, host, 443, uri, true);*/
    char uri[QUARKLINK_MAX_URI_LENGTH];
    /* Database, obtained from enrolling with Quarklink DBS*/
    char database[QUARKLINK_MAX_SHORT_DATA_LENGTH];
    /* DataSource, obtained from enrolling with Quarklink DBS*/
 ...
 * \note 		None.
 * \param[in,out] quarklink the quarklink context, make sure to initialise it with quarklink_init first
 * \retval      QUARKLINK_SUCCESS for success
...
/**
 * \brief       Request a firmware update to Quarklink,
 *              then runs the firmware over the air update.
 ...
 * \param[in,out] quarklink the quarklink context, make sure to initialise it with quarklink_init first
 * \param       signingKey the public key used to validate the firmware, in PEM format. NULL for no key
 ...
 * \param[in] quarklink the quarklink context, make sure to initialise it with quarklink_init first
